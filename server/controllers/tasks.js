...
export const getTasks = async (req, res) => {
    ...
    try {
        ...
        const total = await TaskMessage.countDocuments({});
        const tasks = await TaskMessage.find().sort({ _id: -1 }).limit(LIMIT).skip(startIndex);
...
export const createTask = async (req, res) => {
    ...
    const newTaskMessage = new TaskMessage({ ...task, creator: req.userId, createdAt: new Date().toISOString(), dueDate: new Date().toISOString() })
