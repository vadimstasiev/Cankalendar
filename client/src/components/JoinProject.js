...
import { joinproject, createproject } from '../actions/auth';
...
const JoinProject = (props) => {
    ...
    const submit = async () => {
        dispatch(joinproject({userEmail: user?.result.email, projectId: id}, navigate)).catch((error) => {
            ...
            <div className="flex my-auto items-center justify-center">
                <div className="grid bg-white dark:bg-zinc-900 rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-2/5">
                    <div className="grid grid-cols-1 mt-5 mx-7">
                        <label className="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold pb-5">Please enter the id of the project you want to join</label>
                        <input value={id} onChange={e => setId(e.target.value)} className="py-2 px-3 rounded-lg border-2 border-gray-300 dark:bg-zinc-900 dark:border-zinc-500 text-zinc-700 dark:text-white mt-1 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:border-transparent" type="text" placeholder="Enter the ID" />
                    </div>
...
export {JoinProject, CreateNewProject}